...
contract BlockEstateRevenueDistributorTest is Test {
    ...
    address public complianceOfficer = address(0x10);
    ...
    function setUp() public {
        ...
        propertyToken = new BlockEstatePropertyToken(
            address(router), address(this), "Test Property", "TEST", admin, propertyOwner, maxSupply, sharePrice
        );
    ...
    function mintTokensForInvestors() internal {
        uint256 stableAmount1 = 1000 * 10 ** 6;
        uint256 stableAmount2 = 2000 * 10 ** 6;
        uint256 stableAmount3 = 3000 * 10 ** 6;

        propertyToken.mint(investor1, stableAmount1);
        propertyToken.mint(investor2, stableAmount2);
        propertyToken.mint(investor3, stableAmount3);
    }
    ...
    function testDepositRevenue() public {
        ...
        assertEq(distributor.propertyEscrow(address(propertyToken)), revenueAmount);
    }
    ...
    function testUpdateOnTransfer() public {
        ...
        // Check pending before transfer
        uint256 pendingBefore = distributor.pendingRevenue(address(propertyToken), investor1);
        assertTrue(pendingBefore > 0);

        // Perform transfer
        uint256 transferAmount = propertyToken.balanceOf(investor1) / 2;
        vm.prank(investor1);
        propertyToken.transfer(investor2, transferAmount);

        // Claim rewards for investor1 - this will update their reward debt
        vm.prank(investor1);
        distributor.claim(address(propertyToken));

        // The pending after claim should be 0 (or very small)
        uint256 pendingAfter = distributor.pendingRevenue(address(propertyToken), investor1);

        // After claiming, pending should be 0
        assertEq(pendingAfter, 0);

        // Verify investor2 can also claim (they have rewards too)
        vm.prank(investor2);
        distributor.claim(address(propertyToken));

        // Both investors should have received rewards
        assertTrue(stableToken.balanceOf(investor1) > 10000 * 10 ** 6);
        assertTrue(stableToken.balanceOf(investor2) > 10000 * 10 ** 6);
    }
    ...
    function testDustAccumulation() public {
        propertyToken.mint(investor1, 1234 * 10 ** 6);
        propertyToken.mint(investor2, 2345 * 10 ** 6);
        propertyToken.mint(investor3, 3456 * 10 ** 6);
    ...
    function testClaimWithMultipleInvestors() public {
        ...
        assertTrue(stableToken.balanceOf(investor1) > initialBalance1);
        assertTrue(stableToken.balanceOf(investor2) > initialBalance2);
        assertTrue(stableToken.balanceOf(investor3) > initialBalance3);
    }
}
